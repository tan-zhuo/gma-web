  ...
  if (badge) {
    return logo ? (
      <span className="inline-flex items-center h-7 px-2.5 rounded-md bg-black/55 backdrop-blur border border-white/10" style={{ boxShadow: '0 0 14px var(--brand-glow)' }}>
        <img src={logo.file} alt={BRANDS[brand].label} className="h-3.5 max-w-[64px] w-auto object-contain" style={isMono ? { filter: 'brightness(0) invert(1)' } : undefined} draggable={false} />
      </span>
    ) : (
      <span className="inline-flex items-center h-7 px-2.5 rounded-md font-head text-[10px] tracking-[.15em] text-white" style={{ background: 'var(--brand)', boxShadow: '0 0 14px var(--brand-glow)' }}>{BRANDS[brand].short}</span>
    )
  }
  if (!logo) {
