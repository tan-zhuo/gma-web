...
export function ConsoleCard({ console: c, index, onOpen }: Props) {
  const { t } = useLang()
  return (
    <article data-brand={c.brand} className="card group glass rounded-[20px] overflow-hidden flex flex-col anim-fade-up" style={{ animationDelay: `${Math.min(index, 12) * 40}ms` }}>
      <div className="card-visual h-[200px] max-[720px]:h-[180px] relative flex items-center justify-center overflow-hidden [perspective:900px]">
      ...
      <div className="px-5 pt-4 pb-5 max-[720px]:px-4 flex flex-col gap-3 flex-1">
        <div>
        ...
        <dl className="grid grid-cols-3 gap-2 text-[12px]">
          {([[t.consoles.cpu, c.cpu], [t.consoles.gpu, c.gpu], [t.consoles.ram, c.ram]] as const).map(([k, v]) => (
        ...
        <div className="text-[12px] text-muted flex flex-wrap gap-x-2 gap-y-1 items-center">
          <span className="text-white/60">{t.consoles.games}</span>
        ...
        <div className="mt-auto pt-3 border-t border-line flex items-center justify-between gap-3">
          <div className="text-[12px] text-muted leading-tight">{t.consoles.price}<br /><b className="text-white font-medium text-[13px]">{c.price}</b></div>
          <button type="button" onClick={() => onOpen(c.id)} className="btn btn-accent btn-sm shrink-0">{t.consoles.open} <span className="arr">→</span></button>
        </div>
      </div>
    </article>
  )
